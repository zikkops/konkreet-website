...
export function ProjectProfile({ profile }: { profile: Profile }) {
  ...
  // row to the spec card rather than showing an empty column.
  const hasPhotos = profile.gallery.length > 0;

  return (
    <section
      id={profileSlug(profile.title)}
    ...
    >
      ...
      <div className={`flex flex-col ${hasPhotos ? "lg:w-1/2" : "w-full"}`}>
        <StickyCard>
